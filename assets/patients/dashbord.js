...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (profileForm) {
    profileForm.addEventListener("submit", function (e) {
      ...
      fetch("../../controller/validate_profile_ajax.php", {
        method: "POST",
        body: formData,
      })
        .then((res) => res.json())
        .then((data) => {
          if (data.status === "error") {
            profileMessageDiv.textContent = Object.values(data.errors).join(" ");
            profileMessageDiv.classList.add("message-error");
            profileMessageDiv.style.display = "block";
          } else {
            profileMessageDiv.style.display = "none";
            profileForm.submit(); // Proceed to PHP
          }
        })
        .catch(() => {
          profileMessageDiv.textContent = "Server error.";
          profileMessageDiv.style.display = "block";
        });
    });
  }
  ...
  if (passwordChangeForm) {
    passwordChangeForm.addEventListener("submit", function (e) {
      ...
      fetch("../../controller/validate_password_ajax.php", {
        method: "POST",
        body: formData,
      })
        .then((res) => res.json())
        .then((data) => {
          if (data.status === "error") {
            passwordMessageDiv.textContent = Object.values(data.errors).join(" ");
            passwordMessageDiv.classList.add("message-error");
            passwordMessageDiv.style.display = "block";
          } else {
            passwordMessageDiv.style.display = "none";
            passwordChangeForm.submit(); // Proceed to PHP
          }
        })
        .catch(() => {
          passwordMessageDiv.textContent = "Server error.";
          passwordMessageDiv.style.display = "block";
        });
    });
  }
